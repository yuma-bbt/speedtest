...
var latitudeStatus = '0.0'
var longitudeStatus = '0.0'
var cookieStatus = 'hoge'
...
var settings = {
  ...
  telemetry_level: 1, // 0=disabled, 1=basic (results only), 2=full (results+log)
...
this.addEventListener('message', function (e) {
  ...
  if(typeof(params[0].indexOf(":")) === judgement_value && typeof(params[0].indexOf("latitude")) === judgement_value) {
	...
	if(locations[0] === "latitude"){
       		latitudeStatus  = locations[1];
		tlog("latitude: "+latitudeStatus)
	}
	else if(locations[0] === "longitude"){
       		longitudeStatus = locations[1];
  ...
  if(params[0].indexOf("cookie:") >= 0) {
	var cookievar;
        cookievar = params[0].split(":")
	cookieStatus = cookievar[1];
	tlog("cookie :"+ cookieStatus)
}
  ...
  if (params[0] === 'abort') { // abort command
	...
	testStatus = 5; dlStatus = ''; ulStatus = ''; pingStatus = ''; jitterStatus = ''; latitudeStatus =''; longitudeStatus =''; cookieStatus =''; // set test as aborted
  }
